(function ($) {
  ...
  function isBlockedSshHost(host) {
    ...
    var parts = h.split('.').map(function (p) { return parseInt(p, 10); });
    if (parts[0] === 127) return true;
    if (parts[0] === 10) return true;
    if (parts[0] === 192 && parts[1] === 168) return true;
    if (parts[0] === 172 && parts[1] >= 16 && parts[1] <= 31) return true;
    if (parts[0] === 169 && parts[1] === 254) return true;
    if (parts.every(function (p) { return p === 0; })) return true;
  }
  ...
  function toast(msg, type) {
    ...
    setTimeout(function () { $t.addClass('hidden'); }, 4000);
  }
  ...
  function sshPayload() {
    return {
      ssh_host: $('#ssh_host').val().trim(),
      ssh_password: $('#ssh_password').val(),
      ssh_port: parseInt($('#ssh_port').val(), 10) || 22,
      ssh_user: $('#ssh_user').val().trim() || 'root',
      server_os: $('#server_os').val() || 'generic'
    };
  }
  ...
  function refreshSiteCardIndexes() {
    $('#sites-container .site-card').each(function (index) {
      $(this).find('.site-index').text(index + 1);
  ...
  function collectFormData(extra) {
    ...
    var data = {
      ...
      bt_port: parseInt($('#bt_port').val(), 10) || 8888,
      nginx_version: $('#nginx_version').val(),
      php_version: $('#php_version').val(),
      mysql_version: $('#mysql_version').val(),
      sites: sites
    };
    var btUser = $('#bt_user').val().trim();
    var btPass = $('#bt_password').val();
  ...
  function renderPreflightResult(res) {
    ...
    if (res.blocked || res.domain_conflict) {
      $box.addClass('error').html(
        '<strong>无法继续安装</strong><p>' + esc(res.message) + '</p>' + buildEnvDetailHtml(res)
      );
      return;
    }
    ...
    if (!res.ok) {
      $box.addClass('error').html(
        '<strong>检测未通过</strong><p>' + esc(res.message) + '</p>'
      );
      return;
    }
  ...
  function runPreflight() {
    ...
    if (isBlockedSshHost(payload.ssh_host)) {
      toast(SSH_HOST_REJECT_MSG, 'error');
      return $.Deferred().reject();
    }
    ...
    payload.php_version = $('#php_version').val();
    return $.ajax({
  ...
  function buildEnvDetailHtml(res) {
    ...
    if (res.php_version_fallback && res.php_version_requested && res.php_version_effective) {
      html += '<li>PHP 版本：' + esc(res.php_version_requested) + ' → 将使用 PHP '
        + esc(res.php_version_effective) + '（系统自动降级）</li>';
    }
    html += '<li>宝塔面板：' + (res.baota_installed ? '已安装' : '未安装') + '</li>';
    html += '<li>Web 环境：' + esc(formatWebEnvironment(res.web_environment)) + '</li>';
    if (res.site_dirs > 0) {
    ...
    var planDomains = res.target_domains && res.target_domains.length
      ? res.target_domains
      : (res.target_domain ? [res.target_domain] : []);
    if (planDomains.length) {
      html += '<li>计划绑定域名：' + planDomains.map(function (d) {
  ...
  function renderCheckFlow(res) {
    ...
    var items = [
      { label: 'SSH 连接', status: '成功', detail: $('#ssh_host').val().trim(), state: 'done' },
  ...
  function buildConfirmRows() {
    var nginxVal = $('#nginx_version').val();
    var phpVal = $('#php_version').val();
    var mysqlVal = $('#mysql_version').val();
    var rows = [
      ['SSH 地址', $('#ssh_host').val().trim()],
      ['SSH 端口', String(parseInt($('#ssh_port').val(), 10) || 22)],
      ['SSH 用户名', $('#ssh_user').val().trim() || 'root'],
      ['SSH 密码', maskSecret($('#ssh_password').val())],
      ['系统类型', OS_LABELS[$('#server_os').val()] || $('#server_os').val()],
      ['宝塔用户名', $('#bt_user').val().trim() || '（自动生成）'],
      ['宝塔密码', maskSecret($('#bt_password').val())],
      ['宝塔端口', String(parseInt($('#bt_port').val(), 10) || 8888)],
  ...
  function cancelDeploy() {
    ...
    var message =
      '确定要终止当前部署任务吗？\n\n' +
      '终止后，目标服务器上可能留有未完成的安装残留（如宝塔、PHP、数据库等）。' +
      '建议重新安装系统后再进行部署。\n\n' +
      '是否确认终止？';
    ...
    requestTaskCancel(token).done(function (res) {
      if (!res || res.ok !== true) {
  ...
  function submitDeploy() {
    ...
    $.ajax({
      ...
      var msg = status === 'timeout'
        ? '提交超时，服务器环境验证耗时过长，请稍后重试'
        : '提交失败';
      try {
        var err = xhr.responseJSON;
        if (err.detail) {
          msg = typeof err.detail === 'string' ? err.detail : (err.detail[0].msg || err.detail[0]);
        }
      } catch (e2) { /* ignore */ }
      toast(msg, 'error');
      $('#confirm-panel').removeClass('hidden');
  ...
  function pollDeployOnce(token, callback, catchUpRound) {
    ...
    $.ajax({
      ...
      if (res.truncated && res.skipped > 0 && catchUpRound === 0) {
        appendLog(
          'system',
          '（已省略较早的 ' + res.skipped + ' 条日志，仅显示最近 ' + LOG_FETCH_LIMIT + ' 条）'
        );
      }
  ...
  function appendLog(phase, message) {
    ...
    $viewer.append(
      '<div class="log-line"><span class="phase">[' + esc(phase) + ']</span> ' + esc(message) + '</div>'
    );
  ...
  function buildResultTableRows(rows) {
    ...
    rows.forEach(function (r) {
      ...
      var isLink = r[2] === true;
      var val = isLink
        ? '<a href="' + esc(r[1]) + '" target="_blank" rel="noreferrer">' + esc(r[1]) + '</a>'
        : esc(r[1]);
  ...
  function renderFailedResult(errorMessage, result) {
    ...
    if (result.panel_url) {
      $('#partial-table-baota').html(buildResultTableRows([
    ...
    if (result.database && result.database.name) {
      $('#partial-table-db').html(buildResultTableRows([
  ...
  function renderResult(result) {
    ...
    if (hasAutoPassword) {
      $('#result-password-notice')
        .removeClass('hidden')
        .html('部分网站管理员密码由系统自动生成，请妥善保存，登录后台后请<strong>尽快修改</strong>。');
    } else {
  ...
  function startDeployFlow() {
    ...
    runPreflight().done(function (res) {
      if (!res) return;
  ...
  $(window).on('beforeunload', function (e) {
    ...
    var message = pageGuardMode === 'running'
      ? '当前任务正在进行中，请暂时不要关闭页面，避免任务失败。'
      : '请确认已保存并记录好您的网站信息。关闭后，此信息将无法再次查看。';
    e.preventDefault();
  ...
  $('#ssh_host, #ssh_password, #ssh_port, #ssh_user, #server_os').on('input change', function () {
    lastPreflight = null;
